...
def _set_row_height_cm(row, height_cm: float, rule: str = 'atLeast'):
    """设置行高（cm）。rule 可选 'exact' / 'atLeast' / 'auto'。"""
    tr = row._tr
    trPr = tr.find(qn('w:trPr'))
    if trPr is None:
...
def _fill_photos(doc: Document, photos: dict):
    ...
    布局（旅业 2.3.4+）:
      Table 3 R1: 场所照片  —— 横拍，13×9.5cm，行高 10cm
      Table 3 R3: 经营许可证 —— 横拍，16×10cm，整行合并（不变）
      Table 4 R1: 营业执照  —— 竖拍，10×17cm，行高 17.5cm，居中
      Table 5 R1: 现场记录表 —— 竖拍（不变）
    """
    table3 = doc.tables[3]

    facade_path = photos.get("facade")
    # 场所照片：改为横屏放置（13 cm 宽 × 9.5 cm 高），行高同步收窄到 10 cm。
    if facade_path:
        _clear_cell(table3.rows[1].cells[1])
        _add_image_to_cell(table3.rows[1].cells[1], facade_path,
                           max_width_cm=13.0, max_height_cm=9.5)
        _set_row_height_cm(table3.rows[1], 10.0, rule='atLeast')
    else:
        _set_row_height_cm(table3.rows[1], 10.0, rule='atLeast')

    permit_path = photos.get("permit")
    if permit_path:
        # 经营许可证：模板该行合并为整行单元格 (~16.6cm 宽)，
    ...
    if len(doc.tables) > 4:
        ...
        # 营业执照改为竖屏（portrait）放置：图片最大 10 × 17 cm，
        # 行高同步扩展到 17.5 cm，居中放置在 15.77cm 宽的单元格中。
        if license_path and len(table4.rows) > 1:
            ...
            _add_image_to_cell(table4.rows[1].cells[0], license_path,
                               max_width_cm=10.0, max_height_cm=17.0)
            _set_row_height_cm(table4.rows[1], 17.5, rule='atLeast')
        elif len(table4.rows) > 1:
            _set_row_height_cm(table4.rows[1], 17.5, rule='atLeast')
